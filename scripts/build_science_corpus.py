...
try:
    ...
    raise SystemExit(
        "Biopython is required. Install with `python3 -m pip install -r requirements.txt`."
    ) from exc
...
CONTROLLED_PATTERNS = ("controlled clinical trial", "clinical trial")
...
PARTICIPANT_PATTERN = re.compile(
    r"\b(\d{1,4})\s+(participants|subjects|men|women|adults|athletes)\b",
    re.IGNORECASE,
)
...
def main() -> None:
    ...
    clusters: list[dict[str, Any]] = config.get("clusters", [])
    if not clusters:
